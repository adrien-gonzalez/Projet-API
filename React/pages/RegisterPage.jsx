  ...
  const handleOnSubmit = async (values, actions) => {
    ...
    try {
      ...
      if (typeof data == "object") {
        // console.log(data);
        data.map((d) => {
            if(d.loginExist) {setLoginError(d.loginExist)}
            if(d.invalidEmail) {setEmailError(d.invalidEmail)}
            if(d.invalidPassword) {setPasswordError(d.invalidPassword) }
            if(d.loginEmpty) {setLoginError(d.loginEmpty)}
            if(d.emailEmpty) {setEmailError(d.emailEmpty)}
            if(d.passwordEmpty) {setPasswordError(d.passwordEmpty)}
            if(d.cpasswordEmpty) {setcPasswordError(d.cpasswordEmpty)}
            if(d.emailExist) {setEmailError(d.emailExist)}
            if(d.invalidLogin) {setLoginError(d.invalidLogin)}
            if(d.passwordAndConfirmNotMatch) {setcPasswordError(d.passwordAndConfirmNotMatch)}
        });
      }
      else {
